...
impl Markov {
    ...
    pub fn train(&mut self, raw_text: &str) {
        ...
        for i in 0..words.len() - 3 {
            ...
            let w4 = Arc::clone(&words[i + 3]);

            let key = (w1, w2, w3);

            if key.0.ends_with('.') && key.1.chars().next().map_or(false, |c| c.is_uppercase()) {
                self.start_words.push(key.clone());
            }

            self.chain.entry(key).or_insert_with(Vec::new).push(w4);
        }
    }
    ...
    pub fn generate(&self, length: usize) -> String {
        ...
        let mut result = vec![
            current_key.0.to_string(),
            current_key.1.to_string(),
            current_key.2.to_string(),
        ];
        ...
        for _ in 0..length {
            if let Some(choices) = self.chain.get(&current_key) {
                ...
                current_key = (
                    Arc::clone(&current_key.1),
                    Arc::clone(&current_key.2),
                    Arc::clone(next_word),
                );
            } else {
                break;
...
fn main() {
    let content = fs::read_to_string("ti.txt").expect("Could not read file");
